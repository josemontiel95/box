...
@Component({
  selector: 'app-insertar-documento',
  templateUrl: './insertar-documento.component.html',
  styleUrls: ['./insertar-documento.component.scss']
})
export class InsertarDocumentoComponent implements OnInit {
  global: Global;
  id: string;
  crearMessage: string= "";
  ...
  handleFileInput(files: FileList) {   
    ...
    formData.append('rol_usuario_id',  '1006');
    formData.append('id_loteCorreos',  this.id);
    formData.append('doc',             this.doc);
	 	console.log(formData);
    let search = new HttpParams();
    search.set('function',             'upLoadDoc');
    search.set('token',                this.global.token);
    search.set('rol_usuario_id',       '1006');
    search.set('id_loteCorreos',       this.id);
  ...
  doIt(objeto : any){
    if(objeto.error==0){
      ...
      console.log(objeto);
      setTimeout(()=>{ this.router.navigate(['administrativo/obras/dashboardLote/'+this.id])}, 1500)
